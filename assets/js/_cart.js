jQuery(document).ready(function ($) {
    ...
    $('.wp-travel-add-to-cart-form').submit(function (event) {
        ...
        $.ajax({
            type: "POST",
            url: wp_travel_cart.ajaxUrl,
            data: cart_fields,
            beforeSend: function () { },
            success: function (data) {
                location.href = wp_travel_cart.cartUrl;
            }
        });
    ...
    $('.wp-travel-cart-remove').click(function (e) {
        ...
        if (confirm(wp_travel_cart.strings.confirm)) {
    ...
    $(document).on('click, change', '.wp-travel-pax', function () {
        ...
        if (selectedPax > availablePax) {
            alert(wp_travel_cart.strings.alert.max_pax_alert.replace('{max_pax}', availablePax));
            ...
            alert(wp_travel_cart.strings.alert.min_pax_alert.replace('{min_pax}', minPax));
...
const wptravelcheckout = (shoppingCart) => {
    ...
    wp_travel_cart && wp_travel_cart.timeout(fetch(`${wp_travel_cart.ajaxUrl}?action=wp_travel_get_cart&_nonce=${wp_travel_cart._nonce}`)
        .then(res => {
            res.json()
                .then(result => {
                    toggleCartLoader()
    ...
    cartItems && cartItems.forEach(ci => {
        ...
        _deleteBtn && _deleteBtn.addEventListener('click', e => {
            ...
            if (confirm(_deleteBtn.dataset.l10n)) {
                ...
                wp_travel_cart.timeout(
                    fetch(`${wp_travel_cart.ajaxUrl}?action=wp_travel_remove_cart_item&_nonce=${wp_travel_cart._nonce}&cart_id=${ci.dataset.cartId}`)
                        .then(res => res.json())
                        .then(result => {
        ...
        edit && edit.addEventListener('click', e => {
            ...
            if (cart.trip_data.inventory && cart.trip_data.inventory.enable_trip_inventory === 'yes') {
                ...
                wp_travel_cart.timeout(
                    fetch(`${wp_travel_cart.ajaxUrl}?${qs}&action=wp_travel_get_inventory&_nonce=${wp_travel_cart._nonce}`)
    ...
    cartItems && cartItems.forEach(ci => {
        ...
        _form.addEventListener('submit', e => {
            ...
            wp_travel_cart.timeout(
                fetch(`${wp_travel_cart.ajaxUrl}?action=wp_travel_update_cart_item&cart_id=${cartId}&_nonce=${wp_travel_cart._nonce}`, {
                    method: 'POST',
    ...
    couponBtn && couponField && couponBtn.addEventListener('click', e => {
        ...
        if (couponField.value.length <= 0) {
            ...
            wp_travel_cart.timeout(
                fetch(`${wp_travel_cart.ajaxUrl}?action=wp_travel_apply_coupon&_nonce=${wp_travel_cart._nonce}`, {
                    method: 'POST',
