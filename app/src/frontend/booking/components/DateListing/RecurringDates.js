...
const RecurringRepeator = ( props ) =>  {
    ...
    let activeClass = selectedDateIds.includes( date.id ) && ( ! recurrindDate || ( recurrindDate && sd == rd ) ) ? 'wp-travel-active-date' : '';
    const { enable_time } = date;
    return <tr key={index} className={loadingClass + activeClass}>
...
const RecurringDates = ( props ) => {
    ...
    return <Fragment key={index}>
    {
		...
		{ activeRecurringDates.map( ( recurrindDate, innerIndex ) => {
			...
			return <RecurringRepeator { ...{ ...props, _nomineePricings, date, recurrindDate, innerIndex, _nomineePricings } }  key={innerIndex} />
        } ) }
		</tbody>
    }
