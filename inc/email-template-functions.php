...
function wp_travel_email_template_header( $type, $sentTo ) {
	...
	switch( $type ){
		...
		if ( $sentTo == 'admin' ) {
			...
			$email_header = isset( $settings['booking_admin_template_settings'] ) ? $settings['booking_admin_template_settings'] : $defaults;
	...
	<body style="background: #fcfcfc;color: #5d5d5d;margin: 0;padding: 0;">
		...
		<table class="wp-travel-wrapper" width="600" cellpadding="0" cellspacing="0" style="color: #5d5d5d;font-family: Roboto, sans-serif;margin: auto;"> 
			<tr class="wp-travel-header" style="background: #fff;">			
				<td align="left" style="background: <?php echo esc_attr( $email_header['admin_header_color'] ); ?>;box-sizing: border-box;margin: 0;padding: 20px 25px;"> <!-- Header -->
					<h2 style="color: #fcfffd;font-size: 20px;margin: 0;padding: 0;text-align: center;"><?php echo esc_html( $email_header['admin_title'] ); ?></h2>
				</td> <!-- /Header -->
			</tr>
<?php
...
function wp_travel_email_template_body_content( $type , $sentTo ){
	...
	switch( $type ){
		...
		if ( $sentTo == 'admin' ) {
			...
			$admin_booking_default = 

			$booking_admin_email_content = isset( $settings['booking_admin_template_settings']['email_content'] ) ? $settings['booking_admin_template_settings']['email_content'] : $admin_booking_default;
 ...
 * @return HTML
 */
function wp_travel_booking_admin_default_email_template(){
