			...
			<div id="shopping-cart">
				<div class="cart-summary">
					...
					<div class="cart-header">
						...
						<small class="subtitle"><?php echo esc_html__( 'You have selected ', 'wp-travel' ) . wp_kses_post(  '<strong data-wpt-cart-item-count="">' . esc_html( count( $cart_items ) ). '</strong>' ) . esc_html__( ' items in your cart', 'wp-travel' ); ?></small>
					</div>
					...
					foreach ( $cart_items as $cart_id => $cart_item ) {
						...
						<li class="list-group-item" data-cart-id="<?php echo esc_attr( $cart_id ); ?>">
								...
								<div class="trip-content">
									...
									<!-- <div style="display:none;" class="trip__partial-payment_detail" data-wpt-trip-partial-total="<?php echo esc_attr( $trip_total_partial ); ?>">
										...
										<span>
											...
											if ( $trip_discount ) {
												...
												echo wp_kses_post( wptravel_get_formated_price_currency( $trip_total_partial - $partial_discount ) );

											} else {
												echo wp_kses_post( wptravel_get_formated_price_currency( $trip_total_partial ) );
											}
											?>
										</span>
									</div> -->
									<div class="trip-meta-content">
										...
										foreach ( $cart_pax as $category_id => $detail ) {
											...
											echo '<span><span data-wpt-category-count="' . esc_attr( $category_id ) . '">'.esc_html( $pax ).'</span> x '.esc_html($ctitle).'</span>';
										}
										?>
									</div>
								</div>
							...
							<div class="cart-item-items">
								<a href="javascript:void(0);" class="del-btn" data-l10n="<?php echo esc_attr( __( 'Are you sure you want to remove ', 'wp-travel' ) .$trip_data['title']. __( ' from cart?', 'wp-travel' ) ); ?>"><i class="wt-icon wt-icon-trash-alt" aria-hidden="true"></i> <?php esc_html_e( 'Remove', 'wp-travel' ); ?></a>
							...
							<div class="update-fields-collapse" >
								<form class="wp-travel__cart-item" action="">
									...
									foreach ( $cart_pax as $category_id => $detail ) {
										$category = isset( $categories[ $category_id ] ) ? $categories[ $category_id ] : array(); // undefined offset fixes.
										$ctitle   = isset($category['term_info']['title']) ? $category['term_info']['title'] : '' ;
										$pax      = (int) $detail['pax'];

										$price_per_group = isset( $category['price_per'] ) ? $category['price_per'] : 'group';

										$category_price = isset( $category['is_sale'] ) ? $category['sale_price'] : ( isset( $category['regular_price'] ) ? $category['regular_price'] : '' );
										...
										<div class="wp-travel-form-group" data-wpt-category="<?php echo esc_attr( $category_id ); ?>">
											<label for="adult"><?php echo esc_html( $ctitle ); ?><?php echo ( isset( $category['price_per'] ) && $category['price_per'] == 'group' ) ? '(' . esc_html__( 'Group', 'wp-travel' ) . ')' : ''; ?></label>
											<div>
												<div class="qty-spinner input-group bootstrap-touchspin bootstrap-touchspin-injected">
													<span class="input-group-btn input-group-prepend" >
														<button data-wpt-count-down class="btn edit-pax-selector-qty" type="button"  data-cart="<?php echo esc_attr( $cart_number ) ?>" data-minpax="<?php echo esc_attr( $cart_pricing['min_pax'] ) ?>" data-allpricing="<?php echo esc_html( $cart_item['trip_data']['enable_pax_all_pricing'] ); ?>">-</button>
													</span>
												...
												<span class="prices">
													<?php echo $price_per_group ? '' : ' x <span data-wpt-category-price="' . esc_attr( $category_price ). '">' . wp_kses_post( wptravel_get_formated_price_currency( WpTravel_Helpers_Trip_Pricing_Categories::get_converted_price( $category_price ) ) ). '</span>'; ?>  <strong><?php echo '<span data-wpt-category-total="' . esc_html( $category_total ). '">' . wp_kses_post( wptravel_get_formated_price_currency( WpTravel_Helpers_Trip_Pricing_Categories::get_converted_price( $category_total ) ) ). '</span>'; ?></strong>
												</span>
											</div>
										</div>
										<?php
